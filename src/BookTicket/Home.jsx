...
class Home extends Component {
  ...
  bookTicket = (ghe) => {
    const cloneDangSachGheDangDat = [...this.state.danhSachGheDangDat];
    const foundIndex = cloneDangSachGheDangDat.findIndex(
      (item) => item.SoGhe === ghe.SoGhe
    );

    if (foundIndex === -1) {
      cloneDangSachGheDangDat.push(ghe);
    } else {
      cloneDangSachGheDangDat.splice(foundIndex, 1);
    }

    this.setState({
      danhSachGheDangDat: cloneDangSachGheDangDat,
    });
  };
  ...
  render() {
    return (
      <div className="container mt-3">
        <h1 className="text-center text-warning">ĐẶT VÉ XE BUS - CYBERSOFT</h1>
